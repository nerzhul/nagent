...
run-vulkan: ## Run with the Vulkan GPU backend (requires libvulkan-dev at build time).
	cargo run -p stt-server --release \
	    --features stt-server/real-backend,stt-server/web-agent,stt-server/datetime-agent,stt-server/weather-agent,stt-server/stock-agent,stt-core/whisper-rs-vulkan

.PHONY: run-hipblas
run-hipblas: ## Run with the HIP/ROCm GPU backend (requires ROCm toolchain at build time).
	cargo run -p stt-server --release \
	    --features stt-server/real-backend,stt-server/web-agent,stt-server/datetime-agent,stt-server/weather-agent,stt-server/stock-agent,stt-core/whisper-rs-hipblas

.PHONY: run-cuda
run-cuda: ## Run with the CUDA GPU backend (requires CUDA toolkit at build time).
	cargo run -p stt-server --release \
	    --features stt-server/real-backend,stt-server/web-agent,stt-server/datetime-agent,stt-server/weather-agent,stt-server/stock-agent,stt-core/whisper-rs-cuda
