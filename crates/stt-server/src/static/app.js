...
const state = {
  ...
  pendingAudioTs: [],
  recording: false,
  // Frontend version baked into this exact bundle (read from
  // `/static/version.txt` on page load). Compared against what the
  // server reports at `/api/version`; a mismatch means the user is on
  // a stale tab and should reload.
  selfFrontendVersion: null,
  // True once we've shown the "new version" banner. Stays true until
  // the page is reloaded so we don't keep nagging.
...
// ---- Version detection ------------------------------------------------------
//
// Goal: detect when the server has been rebuilt with newer frontend
// assets while this tab is still open, and offer the user a one-click
// reload so they don't keep talking to a stale UI.
//
// How:
//   1. On page load, fetch `/static/version.txt` — this file is written
//      by the server's `build.rs` next to the rest of the embedded
//      assets and contains the content hash of the bundle this very
//      page was served from. That's "self".
//   2. Also fetch `/api/version` once at load and then every 30s.
//      `/api/version` returns the *current* hash the server is serving.
//      If that hash changes after the page was loaded, a new build has
//      been deployed and the user should reload.
//   3. Show both versions in the header so the user can see at a glance
//      what they are running, and pop a non-blocking banner on mismatch.
//
// The check is best-effort: any fetch failure (offline, server down,
// CSP) is logged to the console and otherwise ignored. We never block
// the recording flow on the version machinery.

const VERSION_POLL_MS = 30_000;
let versionPollId = 0;

async function fetchSelfVersion() {
  // `/static/version.txt` is generated by `build.rs` and re-bundled by
  // `rust-embed`. It is always served from the same origin, so no CORS
  // gymnastics, and it is tiny (one hex string).
  try {
    const r = await fetch("/static/version.txt", { cache: "no-store" });
    if (!r.ok) throw new Error(`status ${r.status}`);
    const text = (await r.text()).trim();
    if (text) {
      state.selfFrontendVersion = text;
      if (frontendVersionSelf) frontendVersionSelf.textContent = text;
    }
  } catch (e) {
...
function maybeShowUpdateBanner(serverFrontend) {
  // We need both sides to compare. If we couldn't load our own hash
  // (e.g. the version.txt file is missing on a pre-feature build), we
  // also have no reliable baseline, so just stay silent.
  if (!state.selfFrontendVersion) return;
  if (state.updateDismissed) return;
  if (state.selfFrontendVersion === serverFrontend) {
...
function startVersionPolling() {
  if (versionPollId !== 0) return;
  versionPollId = setInterval(fetchServerVersion, VERSION_POLL_MS);
}

if (updateBannerReload) {
  updateBannerReload.addEventListener("click", () => {
    // `location.reload()` re-fetches every asset from the server,
    // which now serves the new bundle (and a new version.txt), so the
    // banner disappears on its own after the reload.
    location.reload();
  });
}
...
// Initial probe + start of the periodic check. We do not block the rest
// of the UI on these fetches: the record button / VAD / WS are
// independent and shouldn't be gated on a metadata endpoint. We do
// sequence self-then-server so a mismatch on a stale tab is detected
// on the very first probe rather than waiting for the next 30s tick.
(async () => {
